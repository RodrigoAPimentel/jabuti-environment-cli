...
const { logo } = require(`${JEC_PATH}/src/modules/logo`);
const {
  ansibleAWXConfigInformations,
} = require(`${JEC_PATH}/src/modules/configInformations`);

const { ask, askAWX } = require(`${JEC_PATH}/src/questions/virtualMachine_tool`);
  ...
  async createVirtualMachineTools() {
    ...
    switch (vmTool) {
      case "Ansible-AWX":
        ...
        shell.exec(vagrantCLI({ vmTool, machine_name, machine_number_of_cores, machine_memory_size, public_ip, vars } ));
      ...
      case "Minishift-Windows":
        ...
        shell.cd(
          `${JEC_PATH}/src/files/vmTools/Minishift-Windows/minishift-1.34.2-windows-amd64`
        );
        shell.exec(
          `minishift start --vm-driver virtualbox --host-only-cidr ${cidr}/24`
        );
        break;
    }
  },
