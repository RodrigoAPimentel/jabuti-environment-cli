...
module.exports = {
  async createVirtualMachine() {
    const { machine_name, public_ip, machine_number_of_cores, machine_memory_size, user, password, ssh_key, programs, ports, terminal } = await ask();

    let vars = [user, password];
    ...
    if (terminal) programs.push('ZSH');

    ssh_key ? vars[2] = ["Yes", `'${shell.cat('~/.ssh/id_rsa.pub').stdout.trim()}'`] : vars[2] = ["No", ""];

    console.log("\n  >>>>>>>>>> Creating Virtual Machine. Please Wait .......");

    shell.cd(`${JEC_PATH}/src/files`);

    shell.exec(vagrantCLI({ machine_name, public_ip, machine_number_of_cores, machine_memory_size, programs, ports, vars }));

    machineConfigInformations(machine_name, public_ip, machine_number_of_cores, machine_memory_size, programs, ports, vars);
  },
};
